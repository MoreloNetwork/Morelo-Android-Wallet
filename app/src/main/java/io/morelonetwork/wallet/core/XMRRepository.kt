package io.morelonetwork.wallet.core
...
import io.morelonetwork.wallet.support.ZH_CN
import io.morelonetwork.wallet.support.extensions.getCurrentLocale
import io.morelonetwork.wallet.support.nodeArray
import java.io.File

class XMRRepository(val context: Application = App.instance) {

    fun getKeysFilePath(name: String): String {
        return "${context.filesDir.absolutePath}${File.separator}wallet${File.separator}mrl${File.separator}$name${File.separator}$name.keys"
    }

    fun getWalletFilePath(name: String): String {
        return "${context.filesDir.absolutePath}${File.separator}wallet${File.separator}mrl${File.separator}$name${File.separator}$name"
    }

    private fun generateXMRFile(name: String): File {
        val dir = "${context.filesDir.absolutePath}${File.separator}wallet${File.separator}mrl${File.separator}$name"
	...
	//TODO change node addresses
    fun insertNodes() {
        // 兼容旧版
        val nodes = AppDatabase.getInstance().nodeDao().getSymbolNodes("MRL")
        ...
        val node = AppDatabase.getInstance().nodeDao().getSymbolNode("MRL")
        val locale = context.getCurrentLocale()
        if (locale == ZH_CN && zhNode == null && node != null && node.url == "node.moneroworld.com:18089") {
            // 兼容旧版，修改中文区默认节点
            val filter = AppDatabase.getInstance().nodeDao().getSymbolNodes("MRL")?.filter {
