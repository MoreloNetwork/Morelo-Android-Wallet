...
import io.morelonetwork.morelo.model.*
...
object XMRWalletController {
    ...
    const val TAG = "MRLWalletController"
    ...
    fun createWallet(aFile: File, password: String): io.morelonetwork.wallet.data.entity.Wallet {
    ...
    fun getTransactionHistory(): List<io.morelonetwork.wallet.data.entity.TransactionInfo> {
        val wallet = getWallet() ?: return emptyList()
        val all = wallet.history.all.sortedByDescending { it.timestamp }
        val allInfo = mutableListOf<io.morelonetwork.wallet.data.entity.TransactionInfo>()
        all.forEach {
    ...
    private fun convert(it: TransactionInfo): io.morelonetwork.wallet.data.entity.TransactionInfo {
        return io.morelonetwork.wallet.data.entity.TransactionInfo().apply {
